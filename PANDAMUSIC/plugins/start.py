...
def _is_clone_client(client=None) -> bool:
    try:
...
def help_menu_markup(client=None) -> InlineKeyboardMarkup:
    rows = [
        [
    ...
    # Clone panel only on MAIN bot — cloned bots should not manage clones
...
async def start_message_private(client, message):
    ...
    if message.command and message.command[0].lower() == "help":
        ...
        buttons = help_menu_markup(client)
...
async def help_menu_cb(client, query):
    if await block_cb_if_maintenance(query):
        return
    await _edit_menu(query, help_list_caption(), help_menu_markup(client), client=client)
    await _answer(query, client=client)
