...
def is_clone_client(client) -> bool:
    """True when this Telegram client is a cloned bot, not the main bot."""
    if client is None:
        return False
    main_token = str(getattr(console, "BOT_TOKEN", "") or "").strip()
    this_token = str(getattr(client, "bot_token", "") or "").strip()
    if this_token and main_token and this_token != main_token:
        return True
    try:
    ...
    if not cid:
        try:
            ...
            cid = int(getattr(me, "id", 0) or 0)
        except Exception:
            cid = 0
    if cid and cid in _clone_clients:
        return True
    return False
