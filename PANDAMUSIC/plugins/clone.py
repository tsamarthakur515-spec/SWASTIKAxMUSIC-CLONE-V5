...
def _extract_token(message: Message) -> str:
    ...
    # after /clone — join ALL remaining pieces (fixes line-break tokens)
...
# ── MAIN: /clone  (same filter style as /start) ─────────────────
...
async def clone_cmd(client, message: Message):
    ...
    try:
        ...
        # private only for tokens (security)
        ...
        if not token:
            ...
            return await _reply(
                ...
                "Ya abhi /clone ke baad agli message me sirf token bhejo.\n\n"
                "• /myclones — list\n"
                "• /delclone ID — delete",
            )
...
async def _do_clone(message: Message, token: str):
    """Delete user msg → cloning.... → success with @username."""
    uid = message.from_user.id
    token = _normalize_token(token)

    # 1) status first (so user always sees something)
    status = await _reply(message, "⏳ <b>cloning....</b>")

    # 2) delete user message (hides token)
    ...
    # lazy import — plugin still loads even if clones broken
    try:
